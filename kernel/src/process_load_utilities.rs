...
impl<C: 'static + Chip> DynamicProcessLoader<C> {
    ...
    fn check_overlap_region(
        ...
        // let mut index = 0;
        // let new_process_count = self.find_open_process_slot().ok_or(0, ProcessLoadError::InternalError)?;
            ...
            for (proc_index, value) in procs.iter().enumerate(){
                while proc_index < new_process_count{
                    ...
                    if new_process_end_address >= process_start_address && new_process_end_address <= process_end_address          
                    {
                        ...
                        */
                        // let new_app_data.new_addr = process_end_address.unwrap() + 1 ;
    ...
    fn find_next_available_address(&self,
        ...
        // let app_memory_len = self.app_memory as usize;  // how to get the end of the userspace app?
        // let new_addr = app_start;

        // we want to start with the flash start value and check for the userspace flash area to see if there is enough space 
        // to flash an app without ovelapping with other apps. One we find such a space, we return the address
                ...
                match empty_result{
                    Ok((empty_space, size))=>{
                        if empty_space == true{
                            ...
                            // unimplemented!();
                            let new_process_count = self.find_open_process_slot().unwrap_or_default();  // should never default because we have at least the OTA helper app running
                            // check if there is a remnant app
                            ...
                            self.procs.map(|procs| {
                                for (proc_index, value) in procs.iter().enumerate(){
                                    ...
                                    {
                                        // if new_app.start_addr == unsafe { *self.ptr_process_region_start_address.offset(index.try_into().unwrap()) }
...
impl<C: 'static + Chip> DynamicProcessLoading for DynamicProcessLoader<C> {
    fn setup(&self, app_length: usize) -> Result<(usize, usize), ErrorCode> {
        ...
        //TODO: Get the right flash address from the linkedlist
        //      Add padding before and after the app
            ...
            Ok(()) =>
                {
                    ...
                    // let new_addr_usize: usize = new_start_addr as usize;
